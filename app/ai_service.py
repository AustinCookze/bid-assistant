...
【废标项分类体系】
必须按照以下8个标准类别对废标项进行分类，招标文件中不涉及的类别可以留空：
...
【归类规则】
- 仔细阅读招标文件，将每一条废标条款归类到上述8类中
- 如果某类在招标文件中没有涉及，输出空内容
- 如果一条内容涉及多个类别，归入最主要的类别
- 保持原文完整性，不要截断
...
```json
{
  ...
  "disqualification_items": [
    {"序号": "1", "废标点": "投标人资格项", "招标文件描述": "该类别的完整废标条款内容"},
    {"序号": "2", "废标点": "初审指标", "招标文件描述": "该类别的完整废标条款内容（如招标文件无此内容，可为空）"},
    {"序号": "3", "废标点": "符合性审查指标", "招标文件描述": "该类别的完整废标条款内容"},
    {"序号": "4", "废标点": "实质性响应", "招标文件描述": "该类别的完整废标条款内容"},
    {"序号": "5", "废标点": "废标/无效投标", "招标文件描述": "该类别的完整废标条款内容"},
    {"序号": "6", "废标点": "封装要求", "招标文件描述": "该类别的完整废标条款内容"},
    {"序号": "7", "废标点": "投标文件标书结构", "招标文件描述": "该类别的完整废标条款内容"},
    {"序号": "8", "废标点": "技术废标项", "招标文件描述": "该类别的完整废标条款内容"}
  ],
  "scoring_criteria": [
    {
...
【重要规则】
1. 废标项必须按照8类标准格式输出：投标人资格项、初审指标、符合性审查指标、实质性响应、废标/无效投标、封装要求、投标文件标书结构、技术废标项
2. 仔细阅读招标文件，将每一条废标条款准确归类到上述8类中
3. 如果某类在招标文件中没有涉及，输出空内容或"本招标文件未明确此类要求"
4. 评分标准要按照示例表格的格式输出，包含：评审因素、细分项、分值、评分细则
5. 保持原文的完整性，不要截断内容
"""
