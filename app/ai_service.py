...
class AIService:
    ...
    def _build_phase1_prompt(self, document_text: str) -> str:
        ...
        基于用户提供的 Prompt 设计
        """
        system_role = """你是一位招标文件结构解析专家。你的任务是通过语义分析和模式识别，从不规则的招标文件文本中提取标准化信息。

【重要规则】
1. 条款层级识别：
   - 5.8 是主条款标题（如"废标"）
   - 5.8.1, 5.8.2, 5.8.3... 是子条款，属于 5.8 的内容
   - 提取时必须保留层级关系，子条款内容要完整提取
...
### 0. 文档结构理解
【层级关系处理】
- 招标文件通常使用层级编号：5（章）→ 5.8（节）→ 5.8.1（条款）→ 5.8.1.1（子条款）
- 5.8 是主条款，5.8.1/5.8.2/5.8.3/5.8.4 是它的具体内容
- 提取时必须把主标题和所有子条款合并为一个完整的废标项

示例：
输入："5.8 废标（终止磋商活动） 出现下列情形之一的，将予废标： 5.8.1 需要审批的项目...； 5.8.2 出现影响采购公正的...； 5.8.3 供应商的报价均超过了...； 5.8.4 因重大变故..."
输出：
- title: "5.8 废标（终止磋商活动）"
- content: "出现下列情形之一的，将予废标：5.8.1 需要审批的项目符合专业条件的供应商或者对采购文件作实质响应的供应商不足3家的（服务类项目不足2家）；5.8.2 出现影响采购公正的违法、违规行为的；5.8.3 供应商的报价均超过了采购限价，采购人不能支付的；5.8.4 因重大变故，采购任务取消的。"
- clauses: ["5.8.1 需要审批的项目...", "5.8.2 出现影响采购公正的...", "5.8.3 供应商的报价均超过了...", "5.8.4 因重大变故..."]
...
请严格按照以下 JSON 格式输出分析结果，格式参照提供的示例表格：
...
```json
{
  ...
  "disqualification_items": [
    {
      "序号": "1",
      "废标点": "投标人资格项/初审指标/符合性审查指标/实质性响应/废标无效投标/封装要求/技术废标项",
      "招标文件描述": "完整的废标条款内容，保持原文的完整性，包括所有子条款"
    }
  ],
  "scoring_criteria": [
    {
      "评审因素": "投标报价/技术要求/项目实施/售后服务/项目设计/商务要求",
      "细分项": "技术参数响应/现场演示/项目实施方案/供货业绩等",
      "分值": "20.0",
      "评分细则": "详细的评分标准和要求"
    }
  ],
...
【重要规则】
1. 废标项要按照示例表格的格式输出，包含：序号、废标点、招标文件描述
2. 评分标准要按照示例表格的格式输出，包含：评审因素、细分项、分值、评分细则
3. 保持原文的完整性，不要截断内容
4. 如果某项不存在，可以留空或省略
"""
...
请分析上述招标文件，按输出格式要求返回 JSON 格式的分析结果。确保提取所有废标项、评分标准、商务条款和技术需求。"""
    ...
    def _build_phase2_prompt(self, phase1_result: str) -> str:
        ...
        基于用户提供的 Prompt 设计
        """
        system_role = """你是一位智能投标文件生成器。你接收阶段一提取的结构化数据，根据实际存在的内容动态生成投标文件模板。

【输出格式要求】
生成的模板必须按照以下表格结构组织：

1. 项目基本信息表
2. 废标项清单表（序号、废标点、招标文件描述）
3. 评分标准表（评审因素、细分项、分值、评分细则）
4. 技术参数响应表
5. 投标文件结构清单

如果某类内容在招标文件中未要求，则相应章节标记为"[本项目无此要求]"或省略。"""
